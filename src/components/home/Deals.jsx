...
import { dealPath } from "../../seo/siteConfig";
...
function ServicesHeading() {
  ...
  return (
    <div className="bg-[#FBFBFB] relative overflow-hidden pt-10">
      ...
      <p aria-hidden="true" className="hidden md:block absolute left-0 top-[60px] text-[80px] lg:text-[140px] font-bold text-gray-100 uppercase select-none z-0 tracking-widest leading-none w-screen px-4">
        DEALS
      </p>
      {/* Mobile - in normal flow above content
      <p aria-hidden="true" className="block  md:hidden text-[40px] sm:text-[60px] font-bold text-gray-200 uppercase select-none tracking-widest leading-none px-4 mb-6">
        DEALS
      </p> */}
        ...
        <div className="relative mt-14 w-100%">
          ...
          >
            {dataset?.map((deal) => (
              <Link
                key={deal.id}
                to={dealPath(deal.dealName, deal.id)}
                className="
